...
# Purpose: provide a set of functions useful for creating machine learning
# models with TensorFlow and Keras. The file includes functions for
# transforming and reshaping data, as well as evaluating the model after it
# has been trained.
#
...
def reshape(
    ...
    """Reshapes a given training dataframe (and a given testing dataframe if
    given). The shape goes from being (n_samples, n_features) to becoming
    ...
    if df_test is not None:
        ...
        spec = ''
    if verbose:
        print(f"Dimensionality of{spec} data: {df_train.shape}")
    ...
    if df_test is not None:
        ...
        if verbose:
            print("Dimensionality of reshaped testing data: X_test: "
...
def get_df_hat(
        df: pd.DataFrame,
        y_hat: np.ndarray,
        prediction_cols: list = None) -> pd.DataFrame:
...
def inverse_transform_dataframe(
    ...
    """Inverse transform a given dataframe with a given scaler. The scaler
    supplied must be the same scaler used for initially transforming the data.
    The returned shape is the same as the input shape."""
...
def _reshape_data(
    ...
    bar = filemag.get_progress_bar(
        range_max, bar_desc).start()  # progress bar
...
def get_anomaly_range(df_loss: pd.DataFrame, threshold: float, size: int = 0
    ...
    """Calls the neighborhood checking function with a boolean dataframe where
    loss values above the input threshold is True."""
...
def _get_neighborhood(df_bool: pd.DataFrame,
                      neighborhood: int = 0) -> pd.DataFrame:
    df_neighborhood = pd.DataFrame(df_bool.values)
    """Gets the neighborhood on each side of a selection column through the
    dataframe shift-function. If the 'neighborhood' input value is odd, an
    extra value is added below the selected column, skewing the symmetry by
    one entry."""
...
def get_faulty(
    ...
    """Retrieves faulty data in the specified input with the specified
    parameters. A complete dataframe in the format expected by the program is
    returned, as well as the transformed, reshaped data alongside the
...
def get_absolute_error(df_real: pd.DataFrame, df_pred: pd.DataFrame) -> dict:
    ...
    for col in df_pred.columns:
        ...
        if df_pred.shape.__len__() == 3:
            ...
            absolute_error[col] = np.mean(
                np.abs(df_real[col] - df_pred[col]), axis=1)
...
def _get_variable_thresholds(
        absolute_error: dict,
        threshold_pct: list) -> dict:
    """Calculate threshold values for all prediction columns based on a list
    of desired threshold percentage values."""
...
def get_thresholds(absolute_error: dict, threshold_pct) -> dict:
    """Gets threshold as a list of thresholds or as a uniform threshold, based
    on if the user inputs a list of threshold percentages or a single, uniform
    threshold percentage."""
    if isinstance(threshold_pct, list):
...
def get_performance(
    ...
    """Returns a dataframe consisting of different performance values. This
    can altered to include other performance parameters if desired."""
...
def get_false_anomalies(
    ...
    """Gets the false anomalies detected by the difference between all
    anomalies and true anomalies (which passed the neighborhood test)."""
    all_anomalies = df_loss.index[df_loss > threshold].tolist()
    true_anomalies = df_true_anomalies.index[df_true_anomalies].tolist()
